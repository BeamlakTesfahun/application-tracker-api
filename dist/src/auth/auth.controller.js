...
let AuthController = class AuthController {
    ...
    async register(dto) {
        const user = await this.auth.register(dto.email, dto.password, dto.firstName, dto.lastName);
        return { user };
    }
    async login(dto, res) {
        const result = await this.auth.login(dto.email, dto.password);
        res.cookie(this.auth.getRefreshCookieName(), result.refreshToken, this.cookieOptions());
    ...
    cookieOptions() {
        const days = Number(this.config.get('REFRESH_COOKIE_DAYS') ?? '7');
